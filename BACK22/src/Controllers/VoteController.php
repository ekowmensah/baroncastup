...
class VoteController extends BaseController
{
    ...
    public function processVote($eventSlug = null)
    {
        ...
        try {
            ...
            $paymentData = [
                ...
                'email' => $data['email'] ?? 'voter@baroncast.com',
                'customer_name' => $data['customer_name'] ?? 'BaronCast Voter',
                'tenant_id' => $event['tenant_id'],
            ...
            if ($paymentMethod === 'card') {
                ...
                $paymentData['name'] = $data['customer_name'] ?? 'BaronCast Voter';
                $paymentData['email'] = $data['email'] ?? 'voter@baroncast.com';
    ...
    public function showShortcodeVoting()
    {
        $this->view('voting/shortcode', [
            'title' => 'Vote by Shortcode - BaronCast'
        ]);
    }
    ...
    public function hubtelWebhook()
    {
        try {
            ...
            if (is_array($decodedPayload) && $this->isUssdHubtelWebhook($decodedPayload)) {
                error_log("Hubtel Webhook: Detected USSD transaction, forwarding to USSD fulfillment handler");
                $ussdController = new UssdController();
                return $ussdController->handleServiceFulfillment($decodedPayload);
            }
    ...
    private function isUssdHubtelWebhook(array $payload)
    {
        $data = $payload['Data'] ?? $payload['data'] ?? [];
        if (is_string($data)) {
            $decodedData = json_decode($data, true);
            $data = json_last_error() === JSON_ERROR_NONE ? $decodedData : [];
        }
        ...
        $reference = $data['ClientReference'] ?? $data['clientReference'] ?? $payload['ClientReference'] ?? $payload['clientReference'] ?? null;
        if (!$reference) {
            return false;
        }

        $db = $this->transactionModel->getDatabase();
