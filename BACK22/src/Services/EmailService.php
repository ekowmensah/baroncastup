...
class EmailService
{
    ...
    {
        try {
            $subject = "Account Approved - Welcome to BaronCast!";
    ...
    private function getTenantApprovalTemplate($tenantData)
    {
        ...
        <head>
            ...
            <title>Account Approved - BaronCast</title>
            <style>
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>Great news! Your BaronCast account has been approved and is now ready for use.</p>
                    ...
                    <h3>What's Next?</h3>
                    <ul>
                        <li>Login to your BaronCast dashboard</li>
                    ...
                    <p>Best regards,<br>
                    <strong>The BaronCast Team</strong></p>
                </div>
                
                <div class='footer'>
                    <p>This email was sent to {$tenantData['email']} because your BaronCast account was approved.</p>
                    <p>&copy; " . date('Y') . " BaronCast. All rights reserved.</p>
                </div>
            </div>
        </body>
        </html>";
    ...
    private function getNewTenantNotificationTemplate($tenantData)
    {
        ...
        <head>
            <meta charset='UTF-8'>
            <meta name='viewport' content='width=device-width, initial-scale=1.0'>
            <title>New Tenant Registration - BaronCast</title>
            <style>
                body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
                .container { max-width: 600px; margin: 0 auto; padding: 20px; }
        ...
        <body>
            <div class='container'>
                ...
                <div class='content'>
                    ...
                    <p>Best regards,<br>
                    <strong>BaronCast System</strong></p>
                </div>
                
                <div class='footer'>
                    <p>This is an automated notification from the BaronCast platform.</p>
                    <p>&copy; " . date('Y') . " BaronCast. All rights reserved.</p>
                </div>
            </div>
        </body>
    ...
    private function loadConfig()
    {
        return [
            ...
            'from_email' => $_ENV['FROM_EMAIL'] ?? 'noreply@baroncast.online.gh',
            'from_name' => $_ENV['FROM_NAME'] ?? 'SmartCast',
            'reply_to_email' => $_ENV['REPLY_TO_EMAIL'] ?? 'support@baroncast.online.gh',
            'reply_to_name' => $_ENV['REPLY_TO_NAME'] ?? 'SmartCast Support',
            'support_email' => $_ENV['SUPPORT_EMAIL'] ?? 'support@baroncast.online.gh',
            'super_admin_email' => $_ENV['SUPER_ADMIN_EMAIL'] ?? 'admin@baroncast.online.gh',
    ...
    {
        $subject = "Test Email from BaronCast";
        $body = "
        <h2>Test Email</h2>
        <p>This is a test email from BaronCast to verify email configuration.</p>
