...
class UssdSession extends BaseModel
{
    ...
    private function handleWelcomeState($sessionId, $input)
    {
        ...
        $welcomeMessage = "Welcome to BaronCast!";
        ...
        if ($tenantId) {
            ...
            if ($tenant) {
                if (!empty($tenant['ussd_welcome_message'])) {
                    ...
                    $tenantName = $tenant['name'] ?? 'BaronCast';
    ...
    private function handleMainMenuState($sessionId, $input)
    {
        ...
        switch ($input) {
            ...
            case '0':
                ...
                return $this->createResponse('Thank you for using BaronCast!', true);
    ...
    private function handleEnterShortcodeState($sessionId, $input)
    {
        ...
        if (!$event || !$eventModel->canVote($event['id'])) {
            return $this->createResponse('Event is not active for voting.', true);
        }

        // The shortcode must belong to a tenant that is enabled on the dialed USSD code.
        $dialedUssdCode = $sessionData['ussd_code'] ?? null;
        if (!$dialedUssdCode && !empty($sessionData['service_code'])) {
            $dialedUssdCode = \SmartCast\Helpers\UssdHelper::extractTenantCode($sessionData['service_code']);
        }

        $tenantModel = new Tenant();
        $eventTenant = $tenantModel->find($event['tenant_id']);

        if (
            !$eventTenant ||
            empty($eventTenant['ussd_enabled']) ||
            empty($eventTenant['ussd_code']) ||
            ($dialedUssdCode && (string) $eventTenant['ussd_code'] !== (string) $dialedUssdCode)
        ) {
            return $this->createResponse('Nominee code is not available on this USSD service.', true);
        }
        ...
        if (!$category) {
            return $this->createResponse('Category is not available for voting.', true);
        }
    ...
    private function sendRegistrationLink($sessionId)
    {
        ...
        // $smsService->send($phoneNumber, "Create your event on BaronCast: {$registrationUrl}");
    ...
    private function handleSelectVoteTypeState($sessionId, $input)
    {
        if ($input == '0') {
            ...
            if (empty($sessionData['contestants'])) {
                $this->endSession($sessionId);
                return $this->createResponse('Vote cancelled. Thank you for using BaronCast.', true);
            }
