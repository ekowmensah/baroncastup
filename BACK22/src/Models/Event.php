...
class Event extends BaseModel
{
    ...
    protected $fillable = [
        ...
        'self_nomination_enabled', 'nomination_requires_approval',
        'nomination_start_at', 'nomination_end_at'
    ];
    ...
    public function canAcceptNominations($eventId)
    {
        $event = $this->find($eventId);
        if (!$event) {
            return false;
        ...
        return $this->isNominationOpen($event);
    }

    public function isNominationOpen($event)
    {
        if (empty($event['self_nomination_enabled'])) {
            return false;
        }
        ...
        if (!empty($event['nomination_start_at']) && strtotime($event['nomination_start_at']) > $now) {
            return false;
        }

        if (!empty($event['nomination_end_at']) && strtotime($event['nomination_end_at']) < $now) {
            return false;
        }
