...
class BaronCastPWA {
    ...
    async init() {
        ...
        if ('serviceWorker' in navigator) {
            try {
                const registration = await navigator.serviceWorker.register('/baronprimecast/public/sw.js');
                console.log('BaronCast PWA: Service Worker registered successfully', registration);
                ...
                console.error('BaronCast PWA: Service Worker registration failed', error);
            }
        }
    ...
    initInstallPrompt() {
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            console.log('BaronCast PWA: Install prompt available');
            e.preventDefault();
            deferredPrompt = e;
        ...
        window.addEventListener('appinstalled', () => {
            console.log('BaronCast PWA: App installed successfully');
    ...
    showInstallButton(deferredPrompt) {
        ...
        if (this.isInstalled()) {
            console.log('BaronCast PWA: App already installed, not showing install button');
            return;
        }

        // Don't show if user has dismissed it before
        if (localStorage.getItem('baroncast-install-dismissed') === 'true') {
            console.log('BaronCast PWA: Install button previously dismissed');
            return;
        }
        ...
        if (!document.getElementById('pwa-install-btn')) {
            ...
            installBtn.addEventListener('click', async () => {
                if (deferredPrompt) {
                    ...
                    console.log('BaronCast PWA: Install prompt result:', outcome);
                    
                    if (outcome === 'accepted') {
                        localStorage.setItem('baroncast-install-accepted', 'true');
                    } else {
                        localStorage.setItem('baroncast-install-dismissed', 'true');
                    }
            ...
            console.log('BaronCast PWA: Install button shown, will auto-hide in 30 seconds');
        }
    }
    ...
    dismissInstallButton() {
        console.log('BaronCast PWA: Install button dismissed by user');
        localStorage.setItem('baroncast-install-dismissed', 'true');
        this.hideInstallButton();
    ...
    autoHideInstallButton() {
        const installBtn = document.getElementById('pwa-install-btn');
        if (installBtn) {
            console.log('BaronCast PWA: Auto-hiding install button after 30 seconds');
            this.hideInstallButton();
        }
    }
    ...
    showInstallSuccessMessage() {
        localStorage.setItem('baroncast-install-accepted', 'true');
        this.showNotification('BaronCast installed successfully! 🎉', 'success');
    }
    ...
    showUpdateNotification() {
        ...
        updateBtn.innerHTML = `
            ...
            <p class="mb-2">A new version of BaronCast is available.</p>
    ...
    async requestNotificationPermission() {
        try {
            ...
            if (permission === 'granted') {
                console.log('BaronCast PWA: Notification permission granted');
                this.showNotification('You\'ll now receive voting updates!', 'success');
            }
        } catch (error) {
            console.error('BaronCast PWA: Notification permission error', error);
        }
    }

    initOfflineDetection() {
        window.addEventListener('online', () => {
            console.log('BaronCast PWA: Back online');
            this.showNotification('You\'re back online! 🌐', 'success');
            this.syncOfflineData();
        });

        window.addEventListener('offline', () => {
            console.log('BaronCast PWA: Gone offline');
    ...
    async queueOfflineAction(form) {
        try {
            ...
            console.log('BaronCast PWA: Background sync registered');
            this.showNotification('Your vote will be submitted when you\'re back online.', 'info');
        } catch (error) {
            console.error('BaronCast PWA: Background sync registration failed', error);
        }
    }
    ...
    async syncOfflineData() {
        ...
        if ('serviceWorker' in navigator) {
            try {
                ...
                console.error('BaronCast PWA: Sync failed', error);
            }
        }
    }
    ...
    isInstalled() {
        ...
        const wasAccepted = localStorage.getItem('baroncast-install-accepted') === 'true';
    ...
    resetInstallPrompt() {
        localStorage.removeItem('baroncast-install-dismissed');
        localStorage.removeItem('baroncast-install-accepted');
        console.log('BaronCast PWA: Install prompt state reset');
    }
}

// Initialize PWA when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
    window.baroncastPWA = new BaronCastPWA();
    
    // Add PWA-specific styling if running as PWA
    if (window.baroncastPWA.isPWA()) {
