...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="btn-group" role="group">
        ...
        <a href="<?= ORGANIZER_URL ?>/events/<?= $event['id'] ?>/nominations" class="btn btn-outline-warning">
            <i class="fas fa-user-check me-1"></i>Approve Nominations
            <?php if (!empty($pendingNominationCount)): ?>
                <span class="badge bg-danger ms-1"><?= (int)$pendingNominationCount ?></span>
            <?php endif; ?>
        </a>
...
<div class="alert alert-warning d-flex justify-content-between align-items-center">
    <div>
        ...
        <strong><?= (int)$pendingNominationCount ?> self-nomination<?= (int)$pendingNominationCount === 1 ? '' : 's' ?> awaiting approval.</strong>
    </div>
    <a href="<?= ORGANIZER_URL ?>/events/<?= $event['id'] ?>/nominations?status=pending" class="btn btn-warning btn-sm">
        <i class="fas fa-check me-1"></i>Review Now
    </a>
</div>
<?php endif; ?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <table class="table table-sm">
                            ...
                            <tr>
                                <td><strong>Self-Nomination:</strong></td>
                                <td>
                                    <span class="badge bg-<?= !empty($event['self_nomination_enabled']) ? 'success' : 'secondary' ?>">
                                        <?= !empty($event['self_nomination_enabled']) ? 'Open' : 'Closed' ?>
                                    </span>
                                </td>
                            </tr>
                            <tr>
                                <td><strong>Nomination Close:</strong></td>
                                <td>
                                    <?= !empty($event['nomination_end_at']) ? date('M j, Y H:i', strtotime($event['nomination_end_at'])) : 'Not set' ?>
                                </td>
                            </tr>
                        </table>
                    </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="d-grid gap-2">
                    ...
                    <a href="<?= ORGANIZER_URL ?>/events/<?= $event['id'] ?>/nominations" class="btn btn-outline-warning btn-sm">
                        <i class="fas fa-user-check me-2"></i>Approve Nominations
                        <?php if (!empty($pendingNominationCount)): ?>
                            <span class="badge bg-danger ms-1"><?= (int)$pendingNominationCount ?></span>
                        <?php endif; ?>
                    </a>
        ...
        <div class="card mt-3">
            ...
            <div class="card-body">
                <div class="d-grid gap-2">
                    ...
                    <form method="POST" action="<?= ORGANIZER_URL ?>/events/<?= $event['id'] ?>/toggle-nominations" class="d-grid">
                        <?= $csrfField ?? '' ?>
                        <?php if (!empty($event['self_nomination_enabled'])): ?>
                            <button type="submit" class="btn btn-outline-danger btn-sm" onclick="return confirm('Close self-nominations for this event now?')">
                                <i class="fas fa-lock me-2"></i>Close Nominations
                            </button>
                        <?php else: ?>
                            <button type="submit" class="btn btn-outline-success btn-sm" onclick="return confirm('Open self-nominations for this event now?')">
                                <i class="fas fa-lock-open me-2"></i>Open Nominations
                            </button>
                        <?php endif; ?>
                    </form>
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="nominationUrl" value="<?= APP_URL ?>/events/<?= $event['code'] ?>/nominate" readonly>
                            <button class="btn btn-outline-secondary" onclick="copyUrl('nominationUrl')">
                                <i class="fas fa-copy"></i>
                            </button>
                        </div>
                    </div>
